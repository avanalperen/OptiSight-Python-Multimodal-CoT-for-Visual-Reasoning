...
if os.path.basename(_file_dir) == "codes":
...
# Create a dedicated single-thread executor for heavy ML models to prevent PyTorch 
# thread-local cache fragmentation and OpenMP CPU thrashing over multiple requests.
...
def ensure_bridge_server():
    ...
    if bridge_process is None or bridge_process.poll() is not None:
        ...
        import subprocess
        bridge_script = os.path.join(_file_dir, "qwen35_bridge_server.py")
        bridge_process = subprocess.Popen(
            ["conda", "run", "-n", "qwen35", "python", bridge_script],
...
async def run_grounded_sam(image, device_choice, text_prompt="door threshold, door frame, doorway, open door", scan_type=None):
    ...
    try:
        ...
        # --- FIX: Run blocking model call in a dedicated ML thread to prevent SSE timeout and memory leak ---
        detections = await asyncio.get_running_loop().run_in_executor(
            ml_executor, grounded_sam.predict, padded_image, text_prompt
        )
...
def calculate_angle(x_min, x_max, hfov=90):
    ...
    # Perspective-correct (non-linear) conversion:
...
def get_config_path(file_type, mode="autonomous"):
    folder = "OptiSight CoT" if mode == "autonomous" else mode
    return os.path.join(BASE_DIR, "settings", folder, f"{file_type}.json")

def load_scenarios(mode="autonomous"):
    default_scenarios = {
        'scenario1': {
            'searching': "",
            'finding': "",
    ...
    path = get_config_path("scenarios", mode)
...
def save_scenarios(scenarios, mode="autonomous"):
    try:
...
def load_settings(mode="autonomous"):
    default_settings = {
        "height": 0.0, 
        "height_lock": False, 
    ...
    path = get_config_path("settings", mode)
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
...
def save_settings(settings, mode="autonomous"):
    try:
        current = load_settings(mode)
...
def sync_prompts_to_filesystem(scenarios, mode="autonomous"):
    """Syncs the latest prompts from all scenarios to designated 'best' text files in the new structure."""
    try:
        states = ['searching', 'finding', 'navigating', 'recovering']
        ...
        for scenario_id, scenario_data in scenarios.items():
            ...
            folder = "OptiSight CoT" if mode == "autonomous" else mode
            scenario_dir = os.path.join("prompts", folder, scenario_id)
            os.makedirs(scenario_dir, exist_ok=True)
            
            for state in states:
                content = scenario_data.get(state)
                if content:
                    path = os.path.join(scenario_dir, f"{state}.txt")
                    with open(path, "w", encoding="utf-8") as f:
                        f.write(content)
                        
        logger.info(f"Synced prompts to filesystem for mode: {mode}")
...
def clear_all_snapshots_internal():
    import glob
    for folder in ['photo', 'video', 'live control']:
        save_dir = os.path.join(os.getcwd(), 'temporary', folder)
        if os.path.exists(save_dir):
            ...
            for f in files:
                try:
                    ...
                    pass
            if files:
                logger.info(f"Cleared {len(files)} snapshots in {save_dir}")

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Clear any residual snapshots from previous runs
    clear_all_snapshots_internal()
    ...
    # Clean up snapshots on server exit
    clear_all_snapshots_internal()
...
@app.get("/api/browse")
async def browse_file(type: str = 'image'):
    # Kept for compatibility if needed, but returning error to force new UI
    return {"error": "Use /api/list_dir instead"}

@app.get("/api/list_dir")
async def list_dir_api(type: str = 'image', current_path: str = None):
    # Determine base dir if current_path is not provided
    if type == 'sim':
        base_dir = os.path.join(os.getcwd(), 'habitats')
        exts = ['.glb', '.json']
    elif type == 'image':
    ...
    elif type.startswith('prompt'):
        mode = type.replace('prompt_', '')
        if mode == 'live_control' or mode == 'habitat' or mode == 'sim':
            base_dir = os.path.join(os.getcwd(), 'prompts', 'live control')
        elif mode == 'photo':
            base_dir = os.path.join(os.getcwd(), 'prompts', 'photo')
        elif mode == 'video':
            base_dir = os.path.join(os.getcwd(), 'prompts', 'video')
        else:
            base_dir = os.path.join(os.getcwd(), 'scenario prompts')
        exts = ['.txt', '.md']
    else:
        base_dir = os.getcwd()
        exts = None
        
    target_path = current_path if current_path else base_dir
    if not os.path.exists(target_path):
        target_path = base_dir # fallback
        if not os.path.exists(target_path):
    ...
    try:
        for entry in os.scandir(target_path):
            is_dir = entry.is_dir()
            if not is_dir:
                if exts and not any(entry.name.lower().endswith(ext) for ext in exts):
                    continue
            items.append({
                "name": entry.name,
                "path": entry.path,
...
@app.get("/api/read_prompt")
async def read_prompt_api(path: str):
...
async def get_scenarios_endpoint(mode: str = "autonomous"):
    return load_scenarios(mode)

@app.post("/save_scenarios")
async def save_scenarios_endpoint(request: Request, mode: str = "autonomous"):
    try:
        data = await request.json()
        if save_scenarios(data, mode):
            # Sync to best text files
            sync_prompts_to_filesystem(data, mode)
...
async def clear_habitat_memory():
    ...
    if habitat_controller:
        ...
        if autonomous_navigator:
            autonomous_navigator.collision_flag = False
...
async def save_prompt(content: str = Form(...), folder: str = Form("custom")):
    try:
        # Clean folder name to prevent path traversal, but allow spaces and basic chars
        safe_folder = "".join([c for c in folder if c.isalnum() or c in (' ', '-', '_')]).strip()
        if not safe_folder:
            safe_folder = "custom"
            
        # Ensure target directory exists
        save_dir = os.path.join("prompts", safe_folder)
        os.makedirs(save_dir, exist_ok=True)
            
        # Get list of existing txt files to find the max index
        existing_files = [f for f in os.listdir(save_dir) if f.endswith('.txt')]
        max_index = 0
        ...
        filepath = os.path.join(save_dir, f"{new_index}.txt")
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(f"--- SAVED AT: {timestamp} | INDEX: {new_index} ---\n\n")
            f.write(content)
            
        return {"status": "success", "message": f"Saved to {save_dir}/{new_index}.txt"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
async def load_model_on_demand(device_choice="cpu", model_choice="qwen2-2b", current_mode="habitat"):
    ...
    elif model_choice == "moondream-2b":
        target_model_path = config.MOONDREAM_PATH
    else:
    ...
    try:
        ...
        if model_choice == "qwen2-2b":
            ...
            model = AutoModelForCausalLM.from_pretrained(
                target_model_path,
                trust_remote_code=True,
                torch_dtype=torch.float16 if device_choice == "cuda" else torch.float32,
                device_map=device_choice if device_choice == "cuda" else "cpu"
            )
            processor = AutoTokenizer.from_pretrained(target_model_path)
            if current_mode == "habitat":
                sam_time, sam_device = preload_grounded_sam(device_choice)
            else:
                sam_time, sam_device = 0.0, "N/A"
...
@app.post("/load_model")
async def load_model_endpoint(device_choice: str = Form("cuda"), model_choice: str = Form("qwen2-2b"), current_mode: str = Form("habitat")):
    try:
        info = await load_model_on_demand(device_choice, model_choice, current_mode)
        msg = f"{info['vlm_model']} loaded on {info['vlm_device']} ({info['vlm_time']:.1f}s). "
        if current_mode == "habitat":
            msg += f"{info['sam_model']} pre-loaded on {info['sam_device']} ({info['sam_time']:.1f}s)."
...
async def unload_all_endpoint():
    ...
    if 'habitat_controller' in globals() and habitat_controller is not None:
        try:
            logger.info("Closing habitat_controller on unload_all request...")
            habitat_controller.close()
        except Exception as e:
            logger.error(f"Error closing habitat_controller: {e}")
        habitat_controller = None

    await unload_model_endpoint()
    
    return {"status": "success", "message": "Model and scene unloaded successfully."}
...
async def analyze(
    ...
    if current_mode:
        target_folder = None
        if current_mode == 'habitat' and habitat_submode == 'live_one_shot':
            target_folder = 'live control'
        elif current_mode in ['photo', 'video']:
            target_folder = current_mode
            
        if target_folder:
            import glob
            save_dir = os.path.join(os.getcwd(), 'temporary', target_folder)
            os.makedirs(save_dir, exist_ok=True)
            ...
            snapshot_url = f"/temporary/{target_folder}/{filename}"
    ...
    try:
        ...
        # Ensure prompt is not empty to prevent tensor dimension errors (index -1 out of bounds)
        if not prompt or prompt.strip() == "":
            prompt = "Describe this image in a short sentence."
            logger.info("Empty prompt detected. Defaulting to 'Describe this image in a short sentence.'")
        ...
        if model_choice == "qwen2-2b":
            ...
            final_response = model.answer_question(enc_image, prompt, processor, max_new_tokens=150)
...
async def clear_snapshots_endpoint(request: Request):
    try:
        data = await request.json()
        ...
        target_folder = None
        if target_mode == 'habitat-live':
            target_folder = 'live control'
        elif target_mode in ['photo', 'video']:
            target_folder = target_mode
            ...
            if os.path.exists(save_dir):
                ...
                for f in files:
                    try:
                        ...
                        pass
                logger.info(f"Cleared snapshots in {save_dir}")
        return {"status": "success"}
    except Exception as e:
...
async def list_scenes():
    scene_dir = os.path.join(os.getcwd(), "habitats")
    if not os.path.exists(scene_dir):
        return {"scenes": []}
    scenes = []
    for root, dirs, files in os.walk(scene_dir):
        for f in files:
            if f.endswith('.glb') or f.endswith('.json'):
                rel_path = os.path.relpath(os.path.join(root, f), scene_dir)
                scenes.append(rel_path.replace('\\', '/'))
...
async def init_sim(
    ...
    try:
        if file:
            # Save uploaded file to habitats
            upload_dir = os.path.join(os.getcwd(), "habitats")
            ...
            scene_to_use = os.path.join(os.path.join(os.getcwd(), "habitats"), scene_name)
        ...
        hssd_config_path = os.path.join(os.getcwd(), "habitats", "hssd-hab.scene_dataset_config.json")
        ...
        async with habitat_lock:
            ...
            if habitat_controller is not None:
                try:
                    logger.info("Closing previous habitat_controller instance...")
                    habitat_controller.close()
                except Exception as e:
                    logger.error(f"Error closing previous habitat_controller: {e}")
...
async def check_models():
    results = {
        "qwen2-2b": os.path.exists(config.QWEN_PATH),
        "qwen3.5-0.8b": os.path.exists(config.QWEN35_08B_PATH),
        "qwen3.5-2b": os.path.exists(config.QWEN35_2B_PATH),
        "moondream-2b": os.path.exists(config.MOONDREAM_PATH),
        "sam2": os.path.exists(config.SAM2_PATH),
        "dino": os.path.exists(config.GD_PATH)
    }
    return results
...
async def save_screenshot(request: Request):
    try:
        ...
        existing = glob.glob(os.path.join(screenshots_dir, "*.png"))
        nums = []
        for f in existing:
            try:
        ...
        filename = os.path.join(screenshots_dir, f"{next_num}.png")
    ...
    try:
        ...
        async with habitat_lock:
            ...
            if command == "auto_align" and autonomous_navigator:
                ...
                # "Straight forward" logic: Always advance, but adjust heading if needed
        ...
        if autonomous_navigator:
            ...
            if habitat_controller.collision_count > last_col:
                if autonomous_navigator._is_running:
                    ...
                    logger.warning("Manual collision detected (stream inactive). Skipping recovery sequence.")
...
async def update_height(height: float = Form(...), locked: bool = Form(...), mode: str = Form("autonomous")):
    global habitat_controller
...
async def get_settings_endpoint(mode: str = "autonomous"):
    return load_settings(mode)

@app.post("/save_target")
async def save_target(target: str = Form(...), mode: str = Form("autonomous")):
    save_settings({"autonav_target": target}, mode)
    return {"status": "success"}
...
async def do_inference_for_navigator(prompt, device_choice, model_choice):
    ...
    if autonomous_navigator and autonomous_navigator.state == "FINDING":
        ...
        if "bounding box" not in prompt.lower() and "<box>" not in prompt.lower():
            ...
            if grounded_sam:
                detections = await asyncio.get_running_loop().run_in_executor(
                    ml_executor, grounded_sam.predict, image, "door threshold, door frame, doorway, open door"
                )
    ...
    if model_choice == "qwen2-2b":
        ...
        with torch.no_grad():
            def run_generate():
                return model.generate(**inputs, max_new_tokens=150, do_sample=True, temperature=0.1)
            generated_ids = await asyncio.get_running_loop().run_in_executor(ml_executor, run_generate)
        ...
        def run_encode():
            try:
                ...
                import traceback
                print(f"MOONDREAM ENCODE ERROR: {e}")
                traceback.print_exc()
                raise e
        enc_image = await asyncio.get_running_loop().run_in_executor(ml_executor, run_encode)
        
        def run_answer():
            try:
                return model.answer_question(enc_image, prompt, processor, max_new_tokens=150)
            except Exception as e:
                import traceback
        ...
        answer = await asyncio.get_running_loop().run_in_executor(ml_executor, run_answer)
        ...
        import os, requests, config, uuid, tempfile
        # Use UUID and system temp directory to prevent cross-thread file collisions and clutter
        temp_image_path = os.path.join(tempfile.gettempdir(), f"temp_auto_image_{uuid.uuid4().hex}.jpg")
        image.save(temp_image_path)
        ...
        try:
            def do_request():
                return requests.post("http://127.0.0.1:8001/analyze", json=api_payload, timeout=120)
...
async def save_experiment(request: Request):
    try:
        ...
        # Ensure experiments directory exists using absolute path relative to server.py
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        experiments_dir = os.path.join(base_dir, "experiments")
        os.makedirs(experiments_dir, exist_ok=True)
        
        # Determine the next file number by checking existing files in the directory
        ...
        metrics_lines = [
            "[ METRICS ]",
            "",
            f"Model Used           : {data.get('modelUsed', 'Unknown')}",
