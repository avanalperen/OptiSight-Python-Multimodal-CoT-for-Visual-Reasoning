...
class HabitatController:
    ...
    def plan_path_with_sam_masks(self, obstacle_masks):
        """
        Projects obstacle SAM binary masks to 3D, plans a collision-free heuristic path to the door
        using purely visual 3D vector math (Observational Space), without relying on Habitat's NavMesh.
        """
        def to_list3(v):
        ...
        # Do NOT clear detected_obstacles_3d here! This enables Persistent Spatial Memory across scans.
        self.planned_waypoints = []
        ...
        agent_pos = np.array([float(agent_state.position[0]), float(agent_state.position[1]), float(agent_state.position[2])])
        
        # Determine door target center in 3D Observational Space
        if hasattr(self, 'tracked_3d_points') and self.tracked_3d_points:
            door_points = np.array(self.tracked_3d_points)
            # To prevent floor pixels from pulling the target too close, we take the mean of the further half of the points
            dists = np.linalg.norm(door_points[:, :3] - agent_pos, axis=1)
            median_dist = np.median(dists)
            far_points = door_points[dists >= median_dist]
            if len(far_points) > 0:
                door_center = np.mean(far_points, axis=0)[:3]
            else:
            ...
            door_center = agent_pos + forward_vec * 3.0
            
        # We NO LONGER snap the door_center to the NavMesh!
        # Instead, we project the target to the agent's floor plane (Y-axis) so waypoints don't float into the sky
        target_pos = np.array([door_center[0], agent_pos[1], door_center[2]])
        
        obstacle_detected = False
        reason = "Clear path (Visual Heuristic)"
        turn_suggestion = "right"
        closest_obs_dist = 99.0
        
        blocking_pts = []
        # Increased to 0.45m to provide a wider safety margin. The robot was trying to squeeze through gaps (e.g. 0.36m) that were too tight.
        ...
        for obs_pts in self.detected_obstacles_3d:
            mask_blocked = False
            mask_pts_np = [np.array(pt[:3]) for pt in obs_pts]
            for obs_pt in mask_pts_np:
                # Check distance from obstacle point to the direct line: Agent -> Target
                d = point_to_segment_dist_xz(obs_pt, agent_pos, target_pos)
            ...
            if mask_blocked:
                blocking_pts.extend(mask_pts_np)

        path_blocked = len(blocking_pts) > 0
        
        dist_to_door = np.linalg.norm(target_pos - agent_pos)
        if dist_to_door < 1.5:
            # The robot is very close to the door threshold. 
            # Force path to be clear so it doesn't mistake the door frame for an obstacle and loop infinitely.
            path_blocked = False
            
        if not path_blocked:
            # Direct path is clear
            obstacle_detected = False
            reason = f"Path clear visually (min dist: {min_obs_path_dist:.2f}m)."
            
            # Create a simple linear interpolation of waypoints for visual consistency
            steps = 5
            points = []
            for i in range(steps + 1):
                t = i / steps
                pt = agent_pos * (1 - t) + target_pos * t
                points.append(pt.tolist())
            self.planned_waypoints = points
            self.logical_waypoints = [to_list3(target_pos)]
        else:
            # Path is blocked! Calculate visual detour
            obstacle_detected = True
            reason = f"Obstacle visually blocks path (dist: {min_obs_path_dist:.2f}m). Detour active."
            blocking_pts = np.array(blocking_pts)
            
            # Calculate distances for reporting
            for pt in blocking_pts:
                dist = np.linalg.norm(agent_pos - pt)
                if dist < closest_obs_dist:
                    closest_obs_dist = dist
                    
            # 1. Heading vector from agent to target (XZ plane)
            door_dir_2d = np.array([target_pos[0] - agent_pos[0], target_pos[2] - agent_pos[2]])
            ...
            # Analyze obstacle's spatial footprint relative to the direct path
            obs_pts_2d = blocking_pts[:, [0, 2]]
            agent_2d = np.array([agent_pos[0], agent_pos[2]])
            
            lateral_offsets = np.dot(obs_pts_2d - agent_2d, p)
            longitudinal_offsets = np.dot(obs_pts_2d - agent_2d, u)
            
            mean_lateral = np.mean(lateral_offsets)
            
            # Find where the obstacle starts and ends along our path
            long_min = np.percentile(longitudinal_offsets, 5)
            long_max = np.percentile(longitudinal_offsets, 95)
            
            # Constrain the detour depths to be reasonably between the agent and the door
            long_min = np.clip(long_min, 0.0, door_dir_len * 0.9)
            long_max = np.clip(long_max, 0.0, door_dir_len * 0.9)
            
            # Ensure the two waypoints have minimal separation
            if long_max < long_min + 0.2:
                long_max = long_min + 0.2
            
            # Base points on the direct path, at the front and back of the obstacle
            base_pt_1_2d = agent_2d + u * long_min
            base_pt_2_2d = agent_2d + u * long_max
            
            if mean_lateral < 0:
                # Obstacle is mostly on our left. Detour RIGHT.
                max_right = np.percentile(lateral_offsets, 95)
                # Shift right enough to clear the obstacle's right edge, minimum 35cm shift for safety
                shift = max(max_right + 0.35, 0.35)
                sub_goal_1_2d = base_pt_1_2d + p * shift
                sub_goal_2_2d = base_pt_2_2d + p * shift
                turn_suggestion = "right"
            else:
                # Obstacle is mostly on our right. Detour LEFT.
                min_left = np.percentile(lateral_offsets, 5)
                # Shift left enough to clear the obstacle's left edge, minimum 35cm shift for safety
                shift = min(min_left - 0.35, -0.35)
                sub_goal_1_2d = base_pt_1_2d + p * shift
                sub_goal_2_2d = base_pt_2_2d + p * shift
                turn_suggestion = "left"
                
            sub_goal_1_3d = np.array([sub_goal_1_2d[0], agent_pos[1], sub_goal_1_2d[1]])
            sub_goal_2_3d = np.array([sub_goal_2_2d[0], agent_pos[1], sub_goal_2_2d[1]])
            
            # If the front of the obstacle is behind us or extremely close (we are alongside it), target the BACK corner (W2)
            if long_min < 0.2 and long_max > 0.2:
                target_sub_goal = sub_goal_2_3d
                detour_index = 2
            else:
                target_sub_goal = sub_goal_1_3d
                detour_index = 1
                
            # Create a short Receding Horizon path: Agent -> Target Sub-goal ONLY
            combined_points = []
            
            # Segment 1: Agent -> Target Sub-goal
            steps1 = 3
            for i in range(steps1 + 1):
                t = i / steps1
                pt = agent_pos * (1 - t) + target_sub_goal * t
                combined_points.append(pt.tolist())
                
            self.planned_waypoints = combined_points
            # Increment global session waypoint index
            self.session_waypoint_index += 1
            if not hasattr(self, 'logical_waypoints'):
                self.logical_waypoints = []
            self.logical_waypoints.append(to_list3(target_sub_goal))
            reason = f"Blocked | Rerouting (Waypoint {self.session_waypoint_index})"
            
            return {
                "obstacle_detected": True,
                "distance": closest_obs_dist,
        ...
        if rgb is not None:
            ...
            # (Removed saving visual frame to output.jpg to prevent clutter)
...
if __name__ == "__main__":
    # Test Block
    SCENE = "habitats/skokloster-castle.glb"
