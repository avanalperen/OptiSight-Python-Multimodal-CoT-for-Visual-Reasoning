...
<cmd>COMMAND</cmd>"""
    ...
    async def navigate_stream(self):
        ...
        import uuid
        current_loop_id = uuid.uuid4().hex
        self._current_loop_id = current_loop_id
        self.detour_alignments_completed = 0
        
        try:
            logger.info(f"Autonomous Navigation started in state: {self.state}, Loop ID: {current_loop_id}")
        
            while self._is_running:
                if getattr(self, '_current_loop_id', None) != current_loop_id:
                    logger.warning(f"Overlapping navigate_stream detected. Aborting old loop {current_loop_id}.")
                    break
                ...
                if self.collision_flag or self.state == "RECOVERING":
                    ...
                    if self.recovery_attempts >= 3:
                        yield self.format_sse('log', "CRITICAL FAILURE: 3 consecutive recovery attempts failed. Mission aborting.")
                        yield self.format_sse('failed', "MISSION_FAILED")
                        yield self.format_sse('stopped', True)
                        self.recovery_attempts = 0
                        self._is_running = False
                        break
                    elif self.recovery_attempts == 1 and self.planned_waypoints:
                        self.state = "NAVIGATING"
                ...
                # Just use the state template directly (Core prompt is deprecated for Autonomous)
                        ...
                        if self.planned_waypoints:
                            ...
                            if dist < 0.30:
                                ...
                                if not self.planned_waypoints:
                                    if getattr(self, 'is_receding_horizon', False):
                                        yield self.format_sse('log', "<span style='color:#00ff00'><b>[Receding Horizon]</b> Detour point reached. Turning towards target and rescanning...</span>")
                                        self.is_receding_horizon = False
                                        ...
                                        # DO NOT clear 3D projections here! We want to keep the door threshold in memory.
                                        
                                        # Use existing door projection to mathematically align towards the door without VLM
                                        yield self.format_sse('log', "Aligning to target door using spatial memory...")
                                        max_align_steps = 10
                                        align_steps = 0
                                        while align_steps < max_align_steps:
                                            cmd = self.get_auto_align_command(proj_info)
                                            if cmd == "move_forward":
                                                break # Successfully centered!
                                            move_res = await self.move_callback(cmd)
                                            if move_res:
                                                yield self.format_sse('frame_update', move_res)
                                                proj_info = move_res.get("projection_info")
                                            await asyncio.sleep(0.1)
                                        ...
                                        # Clear locked bbox to force VLM to re-find the door threshold
                                        self.locked_bbox = None
                                        
                                        # Only do SEARCHING after the FIRST waypoint (W1) to refresh odometry.
                                        # For W2 and beyond, skip SEARCHING and go directly to SCANNING_PATH to avoid redundant delays.
                                        # Since we don't have direct access to controller's index here, we can track it.
                                        if not hasattr(self, 'detour_alignments_completed'):
                                            self.detour_alignments_completed = 0
                                        self.detour_alignments_completed += 1
                                        
                                        if self.detour_alignments_completed == 1:
                                            self.state = "SEARCHING"
                                            self.state = "SCANNING_PATH"
                                        break
                                    else:
                            ...
                            if proj_info and proj_info.get("route") and len(proj_info["route"]) > 0:
                                ...
                                if not wp_proj.get("behind"):
                                    ...
                                    # Trigger Stop-and-Go scanning every 15 steps (disabled while following a detour waypoint!)
                                    if self.scenario != "scenario1" and not getattr(self, 'is_receding_horizon', False):
                            ...
                            if "turn" in align_cmd:
                                reason = f"Aligning (Turn Only): {align_cmd} (Target X: {center_x:.2f})"
                                self.last_command = align_cmd
                            ...
                            action = "move_forward"
                            reason = f"Aligned: Moving towards the door (0.25m)"
                ...
                if self.state == "FINDING":
                    ...
                    while not sam_box_match and self._is_running:
                        ...
                        yield self.format_sse('log', "[SAM Output] " + sam_response.replace('\n', ' '))
                        ...
                        if not sam_box_match:
                            if attempts >= 3:
                                ...
                                yield self.format_sse('log', "Executing smooth autonomous 90-degree search turn to escape false positive...")
                                if self.execute_cmds:
                                    try:
                                        for i in range(9):
                                            if self.collision_flag:
                                                break
                                            move_res = await self.move_callback("turn_right_10")
                                            if move_res:
                                                yield self.format_sse('frame_update', move_res)
                                            await asyncio.sleep(0.05)
                                    except Exception as e:
                                        logger.error(f"Navigation error during false positive escape: {e}")
                                    yield self.format_sse('log', "Scan complete. Analyzing new visual field...")
                                    await asyncio.sleep(0.2)
                                self.state = "SEARCHING"
                                break
                            await asyncio.sleep(0.5) 
                ...
                if self.state == "SCANNING_PATH":
                    ...
                    if analysis_res:
                        ...
                        self.is_receding_horizon = obs_info.get("receding_horizon", False)
                ...
                if self.state == "SEARCHING":
                    try:
                        ...
                        logger.error(f"Inference callback failed: {str(e)}", exc_info=True)
            ...
            logger.info(f"Stream cancelled for loop {current_loop_id}")
            yield self.format_sse('stopped', True)
        except Exception as e:
            logger.error(f"FATAL ERROR in navigate_stream: {e}")
            import traceback
            traceback.print_exc()
            yield self.format_sse('error', f"Fatal internal error: {e}")
        finally:
            if getattr(self, '_current_loop_id', None) == current_loop_id:
                self._streaming_active = False
                self._is_running = False
